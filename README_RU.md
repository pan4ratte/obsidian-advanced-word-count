...
Кнопки в заголовке пресета позволяют создать его копию — удобно, чтобы сделать вариант уже настроенного пресета, — и выбрать, какие из его метрик будут отображаться в строке состояния, чтобы строка состояния оставалась компактной, а в правой панели было видно всё.
...
* Метрики в правой панели можно расположить в один или два столбца: кнопка в заголовке панели переключает макет для текущего пресета, и каждый пресет запоминает свой.
...
### 5. Подсчёт в холстах и во встроенных заметках

* **Поддерживаются холсты Obsidian (Canvas).** В холсте счётчики показывают метрики относительно всего документа, а также показатели выделенных карточек и показатели отдельно редактируемой карточки.
* **Подсчёт текста во встроенных заметках.** Опция в настройках позволяет включить в подсчёты заметки, встроенные в текущую. Поддерживаются вложения целой заметки `![[Заметка]]`, заголовка `![[Заметка#Заголовок]]` и блока `![[Заметка#^блок]]`, вложенные друг в друга заметки тоже учитываются, как и вложения внутри карточек холста. Удобно для книги или диссертации, собранной из встроенных глав.

### 6. Установка расширений сообщества: пресетов, метрик и расширенных настроек
...
| Карточки холста | Считает карточки в холсте: все, выделенные или редактируемую. |
| Связи холста | Считает стрелки в холсте. Для выделенных карточек — стрелки, ведущие к ним, от них и между ними. |
| Несвязанные карточки холста | Считает карточки холста без входящих и исходящих стрелок — идеи, ещё не связанные с остальными. |
| Заметки на холсте | Считает разные заметки хранилища, добавленные на холст как карточки. Заметка на нескольких карточках считается один раз. |
...
| Игнорировать карточки заметок в холсте | При подсчёте в холсте не учитывает карточки заметок, чтобы счётчики охватывали только текст, написанный в самом холсте. |
| Считать названия групп и подписи стрелок в холсте | При подсчёте в холсте также учитывает текст названий групп и подписей стрелок. |
