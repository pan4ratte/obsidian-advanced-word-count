...
* **Дублирование пресетов.** В заголовке каждого пресета появилась кнопка, которая создаёт копию пресета со всеми его настройками, предупреждениями и целями.
* **Макет правой панели для каждого пресета.** В заголовке правой панели, рядом с названием пресета, появилась кнопка, которая переключает отображение метрик между одним и двумя столбцами. Макет запоминается отдельно для каждого пресета.

### Улучшения и исправления ошибок

* Общая настройка «Макет счётчиков в правой панели» удалена: теперь макет выбирается кнопкой в заголовке правой панели. После обновления все пресеты отображаются в два столбца — при необходимости переключите нужные пресеты на один столбец.
* Индикаторы «Круг прогресса» в правой панели теперь стоят на одной линии с числом метрики, выровнены по нему по высоте и стали чуть толще.
